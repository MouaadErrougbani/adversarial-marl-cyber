...
def evaluate_one_episode(cyborg, wrapped_cyborg, agents, write_to_file, i, tot):
    observations, _ = wrapped_cyborg.reset()
    r = []
    a = []
    o = []
    for _ in tqdm(range(500), desc=f"({i + 1}/{tot})"):
        actions = {
...
def run_eval(cfg):
    output_path = os.path.abspath(cfg["eval"]["output_path"])
    if not output_path.endswith("/"):
        output_path += "/"

    if cfg["eval"]["append_timestamp"]:
        output_path += time.strftime("%Y%m%d_%H%M%S") + "/"

    rmkdir(output_path)

    submission = load_submission(os.path.abspath(""))

    if cfg["eval"]["distribute"] == 1:
        run_evaluation(
            submission,
            max_eps=cfg["eval"]["max_eps"],
...
def main_legacy():
    ...
    parser.add_argument("--seed", type=int, default=None, help="Set the seed for CybORG")
